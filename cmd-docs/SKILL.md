...
> **ECC Command** — Gọi bằng \@cmd-docs\ trong Antigravity, hoặc \/docs\ trong Claude Code.
...
1. **Resolve library ID** â€” Call the Context7 tool `resolve-library-id` with the library name and the user's question to get a Context7-compatible library ID (e.g. `/vercel/next.js`).
2. **Query docs** â€” Call `query-docs` with that library ID and the user's question.
3. **Summarize** â€” Return a concise answer and include relevant code examples from the fetched documentation. Mention the library (and version if relevant).
