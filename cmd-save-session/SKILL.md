...
description: "Save current session state to a dated file in ~/.claude/sessions/ so work can be resumed in a future session with full c..."
...
> **ECC Command** — Gọi bằng \@cmd-save-session\ trong Antigravity, hoặc \/save-session\ trong Claude Code.
...
Capture everything that happened in this session â€” what was built, what worked, what failed, what's left â€” and write it to a dated file so the next session can pick up exactly where this one left off.
...
Write every section honestly. Do not skip sections â€” write "Nothing yet" or "N/A" if a section genuinely has no content. An incomplete file is worse than an honest empty section.
...
know it works â€” test passed, ran in browser, Postman returned 200, etc.
Without evidence, move it to "Not Tried Yet" instead.]

- **[thing that works]** â€” confirmed by: [specific evidence]
- **[thing that works]** â€” confirmed by: [specific evidence]

If nothing is confirmed working yet: "Nothing confirmed working yet â€” all approaches still in progress or untested."
...
- **[approach tried]** â€” failed because: [exact reason / error message]
- **[approach tried]** â€” failed because: [exact reason / error message]
...
| File              | Status         | Notes                      |
| ----------------- | -------------- | -------------------------- |
| `path/to/file.ts` | âœ… Complete    | [what it does]             |
| `path/to/file.ts` | ðŸ”„ In Progress | [what's done, what's left] |
| `path/to/file.ts` | âŒ Broken      | [what's wrong]             |
| `path/to/file.ts` | ðŸ—’ï¸ Not Started | [planned but not touched]  |
...
- **[decision]** â€” reason: [why this was chosen over alternatives]
...
[If not known: "Next step not determined â€” review 'What Has NOT Been Tried Yet'
...
[Only fill this if relevant â€” commands needed to run the project, env vars
...
- **`/api/auth/register` endpoint** â€” confirmed by: Postman POST returns 200 with user
  object, row visible in Supabase dashboard, bcrypt hash stored correctly
- **JWT generation in `lib/auth.ts`** â€” confirmed by: unit test passes
  (`npm test -- auth.test.ts`), decoded token at jwt.io shows correct payload
- **Password hashing** â€” confirmed by: `bcrypt.compare()` returns true in test

---

## What Did NOT Work (and why)

- **Next-Auth library** â€” failed because: conflicts with our custom Prisma adapter,
  threw "Cannot use adapter with credentials provider in this configuration" on every
  request. Not worth debugging â€” too opinionated for our setup.
- **Storing JWT in localStorage** â€” failed because: SSR renders happen before
...
| File                             | Status         | Notes                                           |
| -------------------------------- | -------------- | ----------------------------------------------- |
| `app/api/auth/register/route.ts` | âœ… Complete    | Works, tested                                   |
| `app/api/auth/login/route.ts`    | ðŸ”„ In Progress | Token generates but not setting cookie yet      |
| `lib/auth.ts`                    | âœ… Complete    | JWT helpers, all tested                         |
| `middleware.ts`                  | ðŸ—’ï¸ Not Started | Route protection, needs cookie read logic first |
| `app/login/page.tsx`             | ðŸ—’ï¸ Not Started | UI not started                                  |

---

## Decisions Made

- **httpOnly cookie over localStorage** â€” reason: prevents XSS token theft, works with SSR
- **Custom auth over Next-Auth** â€” reason: Next-Auth conflicts with our Prisma setup, not worth the fight
...
Then test with Postman â€” the response should include a `Set-Cookie` header.
```
...
- Each session gets its own file â€” never append to a previous session's file
- The "What Did NOT Work" section is the most critical â€” future sessions will blindly retry failed approaches without it
