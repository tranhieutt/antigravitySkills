...
> **ECC Command** — Gọi bằng \@cmd-sessions\ trong Antigravity, hoặc \/sessions\ trong Claude Code.
...
if (result.success) {
  console.log('âœ“ Alias created: ' + aliasName + ' â†’ ' + session.filename);
} else {
...
if (result.success) {
  console.log('âœ“ Alias removed: ' + aliasName);
} else {
  console.log('âœ— Error: ' + result.error);
  process.exit(1);
}
