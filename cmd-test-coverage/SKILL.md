...
description: "ECC slash command: /test-coverage"
...
> **ECC Command** — Gọi bằng \@cmd-test-coverage\ trong Antigravity, hoặc \/test-coverage\ trong Claude Code.
...
1. **Happy path** â€” Core functionality with valid inputs
2. **Error handling** â€” Invalid inputs, missing data, network failures
3. **Edge cases** â€” Empty arrays, null/undefined, boundary values (0, -1, MAX_INT)
4. **Branch coverage** â€” Each if/else, switch case, ternary

### Test Generation Rules

- Place tests adjacent to source: `foo.ts` â†’ `foo.test.ts` (or project convention)
...
2. Re-run coverage â€” verify improvement
...
- API endpoint handlers (request â†’ response flow)
