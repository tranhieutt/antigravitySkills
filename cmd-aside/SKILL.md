---
name: cmd-aside
description: "Answer a quick side question without interrupting or losing context from the current task. Resume work automatically aft..."
...
> **ECC Command** — Gọi bằng \@cmd-aside\ trong Antigravity, hoặc \/aside\ trong Claude Code.
...
Ask a question mid-task and get an immediate, focused answer â€” then continue right where you left off. The current task, files, and context are never modified.
...
- Keep it short â€” if a full explanation is needed, offer to go deeper after the task
...
- If answering requires reading a file, read it â€” but read only, never write
...
â€” Back to task: [one-line description of what was being done]
```
...
âš ï¸ Note: This answer suggests [issue] with the current approach. Want to address this before continuing, or proceed as planned?
```
...
```
Wait for the user's answer â€” do not make assumptions.
...
â€” Back to task: no active task to resume
```
...
ðŸ“ Worth fixing: [what should be changed]. I'll flag this after the current task unless you want to address it now.
```

**Question is ambiguous or too vague:**
Ask one clarifying question â€” the shortest question that gets the information needed to answer. Do not ask multiple questions.
...
exponential backoff (250ms â†’ 500ms â†’ 1s). It only retries on 5xx responses and
network errors â€” 4xx errors are treated as final and not retried.

â€” Back to task: refactoring the auth middleware in src/middleware/auth.ts
```
...
No â€” the shared cache object in src/cache/store.ts:34 is mutated without locking.
...
âš ï¸ Note: This could affect the feature we're building. Want to address this now or continue and fix it in a follow-up?
```
...
- Never modify files during an aside â€” read-only access only
- The aside is a conversation pause, not a new task â€” the original task must always resume
