---
name: cmd-resume-session
description: "Load the most recent session file from ~/.claude/sessions/ and resume work with full context from where the last session..."
...
> **ECC Command** — Gọi bằng \@cmd-resume-session\ trong Antigravity, hoặc \/resume-session\ trong Claude Code.
...
CURRENT STATE:
âœ… Working: [count] items confirmed
ðŸ”„ In Progress: [list files that are in progress]
ðŸ—’ï¸ Not Started: [list planned but untouched]

WHAT NOT TO RETRY:
[list every failed approach with its reason â€” this is critical]
...
[if not defined: "No next step defined â€” recommend reviewing 'What Has NOT Been Tried Yet' together before starting"]
...
If the next step is clearly defined in the session file and the user says "continue" or "yes" or similar â€” proceed with that exact next step.

If no next step is defined â€” ask the user where to start, and optionally suggest an approach from the "What Has NOT Been Tried Yet" section.
...
Note this during the briefing â€” "âš ï¸ `path/to/file.ts` referenced in session but not found on disk."

**Session file is from more than 7 days ago:**
Note the gap â€” "âš ï¸ This session is from N days ago (threshold: 7 days). Things may have changed." â€” then proceed normally.

**User provides a file path directly (e.g., forwarded from a teammate):**
Read it and follow the same briefing process â€” the format is the same regardless of source.
...
PROJECT: my-app â€” JWT Authentication
...
CURRENT STATE:
âœ… Working: 3 items (register endpoint, JWT generation, password hashing)
ðŸ”„ In Progress: app/api/auth/login/route.ts (token works, cookie not set yet)
ðŸ—’ï¸ Not Started: middleware.ts, app/login/page.tsx

WHAT NOT TO RETRY:
âŒ Next-Auth â€” conflicts with custom Prisma adapter, threw adapter error on every request
âŒ localStorage for JWT â€” causes SSR hydration mismatch, incompatible with Next.js

OPEN QUESTIONS / BLOCKERS:
- Does cookies().set() work inside a Route Handler or only Server Actions?

NEXT STEP:
In app/api/auth/login/route.ts â€” set the JWT as an httpOnly cookie using
...
- Never modify the session file when loading it â€” it's a read-only historical record
- The briefing format is fixed â€” do not skip sections even if they are empty
- "What Not To Retry" must always be shown, even if it just says "None" â€” it's too important to miss
